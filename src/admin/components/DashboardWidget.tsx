...
export const dashboardWidgetCatalog: DashboardWidgetDefinition[] = [
  {
    ...
    description:
      "Immediate work, requests, messages, expenses, and notices.",
    defaultSize: "tall"
  },
  {
    type: "metrics",
    title: "Metrics",
    description: "Condensed financial and operational performance.",
    defaultSize: "medium"
  },
  {
    type: "quick-forms",
    title: "Quick Forms",
    description: "Fast access to commonly used forms.",
    defaultSize: "small"
  },
  {
    type: "upcoming-jobs",
    title: "Upcoming Jobs",
    description: "The next scheduled jobs across Pioneer businesses.",
    defaultSize: "medium"
  },
  {
    ...
    description:
      "Jobs, revenue, expenses, estimates, and changes this week.",
    defaultSize: "medium"
  },
  {
    type: "open-estimates",
    title: "Open Estimates",
    description: "Draft and sent estimates awaiting action.",
    defaultSize: "small"
  },
  {
    type: "outstanding-expenses",
    title: "Outstanding Expenses",
    description: "Pending and high-value expense records.",
    defaultSize: "small"
  },
  {
    type: "notifications",
    title: "Notifications",
    description: "Unread and urgent notifications.",
    defaultSize: "small"
  },
  {
    type: "recent-activity",
    title: "Recent Activity",
    description: "Recent administrative activity across the system.",
    defaultSize: "medium"
  },
  {
    type: "business-health",
    title: "Business Health",
    description: "Status summary for every Pioneer division.",
    defaultSize: "medium"
  }
];
  ...
  switch (widget.type) {
    case "today-snapshot":
      return (
        <div className="dashboard-widget__snapshot-grid">
          ...
          <Link to={ROUTES.admin.notifications}>
            <strong>0</strong>
            <span>Urgent Messages</span>
          </Link>
          <Link to={ROUTES.admin.expenses}>
            <strong>0</strong>
            <span>Expenses Over $500</span>
          </Link>
          <Link to={ROUTES.admin.notifications}>
            <strong>0</strong>
    ...
    case "quick-forms": {
      ...
      const selectedForms = selectedFormIds
        ...
        .filter((form): form is (typeof formDefinitions)[number] =>
          Boolean(form)
        )
      ...
      if (selectedForms.length === 0) {
        return (
          <WidgetEmpty
            text="No quick forms are selected. Configure this widget to add forms."
            link={ROUTES.admin.forms}
            label="Open Forms Library"
          />
        );
      }
    ...
    case "upcoming-jobs":
      return (
        <WidgetEmpty
          text="Upcoming jobs will appear here once scheduling data is connected."
  ...
  return (
    <article
      className={`dashboard-widget dashboard-widget--${widget.size}${
        dragging ? " dashboard-widget--dragging" : ""
      }`}
      draggable={editing}
    ...
    >
      <header className="dashboard-widget__header">
        ...
        {editing ? (
          <span className="dashboard-widget__drag-handle" title="Drag to move">⋮⋮</span>
        ) : null}
      </header>
        ...
        <div className="dashboard-widget__editor">
          <label>
            ...
            <select
              value={widget.type}
              onChange={(event) =>
                onChangeType(event.target.value as DashboardWidgetType)
              }
            >
          ...
          <label>
            ...
            <select
              value={widget.size}
              onChange={(event) =>
                onChangeSize(event.target.value as DashboardWidgetSize)
              }
            >
